...
Save important information with `update_notes` before requesting a new context.
...
content from the active branch, but it excludes extension custom entries, credentials, request
headers, remote checkpoint details, encrypted checkpoint bytes, and unrelated extension state.
...
separator. The tool does not read notes; use `recall_context` for that.
...
flowchart TD
    ...
    H --> I[Use recall_context for older plaintext data]
```
...
Anyone who can read the session can read those notes. `recall_context` sends selected history or notes
to the active model provider as a tool result. Do not store secrets that should not reach that
provider.

Retrieved text is bounded and terminal controls are removed at the result boundary. The extension
never places provider credentials, authorization headers, or remote opaque payloads in recall output.
