...
export function loadContextLineage(entries: readonly SessionEntry[]): ContextLineage | undefined {
	...
	for (const entry of entries) {
		...
		lineage = parseExperimentalContextDetails(entry.details) ?? lineage;
	}
	return lineage;
}
...
export function contextContract(lineage: ContextLineage): string {
	return [
		...
		"Use get_context_remaining to inspect capacity, recall_context to retrieve older history or notes, and update_notes to preserve durable working memory.",
		"Call start_new_context when a fresh context is needed. Important information is not summarized automatically.",
	].join("\n");
}
...
export function reconcileContextContract(
	...
	while (
		...
		(messages[summaryBoundary].role === "compactionSummary" ||
			messages[summaryBoundary].role === "branchSummary")
	) {
		summaryBoundary += 1;
	}
	const contract = createContextContractMessage(lineage);
	return summaryBoundary > 0
		? [...messages.slice(0, summaryBoundary), contract, ...messages.slice(summaryBoundary)]
		: [...messages, contract];
}
