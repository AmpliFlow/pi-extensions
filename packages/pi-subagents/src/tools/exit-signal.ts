...
import { writeSubagentExitSidecar } from "../session/exit-sidecar.js";
import {
	SUBAGENT_COMPLETION_ENTRY,
	SUBAGENT_CONTEXT_PRESSURE_FAILURE_REASON,
	SUBAGENT_CONTEXT_PRESSURE_REASON,
} from "./context-reminders.js";
import type { FinalContextSnapshot } from "./final-context-snapshot.js";
...
export function createExitSignalWriter(deps: ExitSignalWriterDeps) {
	return function writeExitSignal(
		...
		const { isNormalCompletion, contextPressure, failedWhileSpent } = classifyExitPayload(
			deps,
			payload,
			opts,
		);
