import type { OrchestratorSnapshot } from "../../runtime/orchestrator-controller.js";
import { fitLine, renderScrollbar, wrapPlainText } from "./render-helpers.js";
import type { OrchestratorActionId, Theme } from "./render-types.js";
...
export function getOrchestratorActions(snapshot: OrchestratorSnapshot): OrchestratorAction[] {
	const sessionBlocked = snapshot.blockedReason !== null;
	return [
		{
			id: "session-toggle",
			label: snapshot.currentMode ? "Disable in this session" : "Enable in this session",
		...
		},
		{
			...
			disabled: sessionBlocked,
			targetMode: !snapshot.currentMode,
			description: "Fresh: empty conversation; old session remains available via /resume.",
		},
		{
			id: "default-toggle",
			label: "Default for new sessions: " + modeLabel(snapshot.savedGlobalDefault),
			disabled: false,
		},
	];
}

export function getOrchestratorGuardMessage(snapshot: OrchestratorSnapshot): string | undefined {
	...
	const subject =
		snapshot.blockedReason === "running-subagents"
			...
			: snapshot.blockedReason === "parent-busy"
				? "the parent is busy"
				: "pending messages are queued";
	return "Session changes unavailable: " + subject + "; wait or stop explicitly.";
}
...
export function renderOrchestrator(
	...
	const statusLines = selectStatusLines(snapshot, layout.statusLines, height, width);
...
export function keepOrchestratorSelectionVisible(
	...
	if (!range) return 0;
	const statusLines = selectStatusLines(snapshot, layout.statusLines, maxHeight, width);
	const height = Math.max(1, maxHeight - statusLines.length);
	if (range.start < scroll) return range.start;
	if (range.start >= scroll + height) return Math.max(0, range.start - height + 1);
...
export function renderOrchestratorConfirmation(
	...
	const buttons = "  " + selected(" Cancel ", !confirmed) + "  " + selected(" Start ", confirmed);
	const lines: string[] = [
...
function buildLayout(
	...
	const addWrapped = (text: string, tone?: string, actionIndex?: number, prefix = "") => {
		const wrapped = wrapPlainText(text, Math.max(8, width - prefix.length), MAX_WRAP_LINES);
	...
	if (width >= 32) addStatus("Delegate-only: coordination tools only.", "muted");
	...
	for (const [index, action] of getOrchestratorActions(snapshot).entries()) {
		...
		addWrapped(action.label + suffix, action.disabled ? "dim" : "text", index, "  ");
		const labelEnd = lines.length;
		if (action.description) addWrapped(action.description, "muted", index, "    ");
	...
	if (globalAction) {
		const start = lines.length;
		const globalLabel =
			width < 20 ? `Default: ${modeLabel(snapshot.savedGlobalDefault)}` : globalAction.label;
	...
	if (snapshot.effectiveGlobalDefaultSource === "env") {
		addWrapped("PI_ORCHESTRATOR_MODE override is controlling the effective default.", "warning");
	} else if (snapshot.effectiveGlobalDefaultSource === "invalid-env") {
		addWrapped("PI_ORCHESTRATOR_MODE override is invalid; using the safe default.", "warning");
	}
	addWrapped("Changing this default does not switch the current session.", "muted");
...
function selectStatusLines(
	...
	const maxStatusLines = height > 1 ? Math.max(1, Math.min(4, Math.max(0, height - 3))) : 0;
...
function compactGuardStatus(snapshot: OrchestratorSnapshot, width: number): string {
	if (width < 32) {
...
function renderGuardStatus(snapshot: OrchestratorSnapshot, width: number): string {
	...
	if (snapshot.blockedReason === "parent-busy") return "Blocked: parent busy; wait or stop.";
...
function clampScroll(scroll: number, totalLines: number, height: number): number {
