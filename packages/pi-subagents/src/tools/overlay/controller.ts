...
import type { OrchestratorController } from "../../runtime/orchestrator-controller.js";
import { type ResumeServiceRuntime, resumeSubagentSession } from "../../runtime/resume-service.js";
import { completedSubagentResults } from "../../runtime/state.js";
import type { RunningSubagent, SubagentResult } from "../../types.js";
import { buildAgentItems, buildCompletedItems, buildRunningItems } from "./data.js";
import {
...
import { getMaxScroll, renderDetail } from "./render-detail.js";
import { getFooterHints, renderFooter, renderHeader } from "./render-frame.js";
import { fitLine } from "./render-helpers.js";
import { getItemRowCount, renderList } from "./render-list.js";
import type {
	OverlayContext,
	OverlayItem,
	OverlayState,
...
import { TABS } from "./render-types.js";
...
export class SubagentsOverlayController implements Component {
	...
	handleInput(data: string): void {
		...
		if (this.state.view.kind === "detail") this.handleDetailInput(data);
			...
			this.handleOrchestratorConfirmInput(data);
	...
	private moveOrchestratorSelection(direction: -1 | 1): void {
		...
		this.state.orchestrator.selectedIndex = Math.max(
			0,
			Math.min(maxIndex, this.state.orchestrator.selectedIndex + direction),
		);
		this.state.orchestrator.scroll = keepOrchestratorSelectionVisible(
			snapshot,
			this.state.orchestrator.selectedIndex,
	...
	private activateOrchestratorAction(): void {
		...
		this.state.orchestrator.error = result.ok
			? undefined
			: (result.error ??
				getOrchestratorGuardMessage(result.snapshot) ??
				"The orchestrator setting was not changed.");
		this.state.orchestrator.scroll = keepOrchestratorSelectionVisible(
			result.snapshot,
			this.state.orchestrator.selectedIndex,
			this.state.orchestrator.scroll,
	...
	private visibleTabs() {
		...
		const tabs = TABS.filter(
			(tab) => tab.id !== "orchestrator" || Boolean(snapshot && !snapshot.isChildSession),
		);
