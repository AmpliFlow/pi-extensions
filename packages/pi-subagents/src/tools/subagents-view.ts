...
import { completedSubagentResults, runningSubagents } from "../runtime/state.js";
import { appendOrchestratorSessionState } from "../session/orchestrator-state.js";
import {
	type OverlayRuntime,
	SubagentsOverlayController,
	type SubagentsOverlayResult,
} from "./overlay/index.js";
import { getOrchestratorGuardMessage } from "./overlay/orchestrator-view.js";

export { SubagentsOverlayController as SubagentsOverlay } from "./overlay/index.js";
...
export function registerSubagentsView(pi: ExtensionAPI, runtime: OverlayRuntime) {
	...
	async function open(ctx: ExtensionContext): Promise<SubagentsOverlayResult> {
		...
		const customPromise = ctx.ui.custom<SubagentsOverlayResult>(
	...
	async function startFreshSession(
		...
		try {
			...
			if (snapshot.blockedReason) {
				ctx.ui.notify(
					getOrchestratorGuardMessage(snapshot) ??
						"Wait for the current work to finish or stop it explicitly.",
					"warning",
				);
				return;
			}
