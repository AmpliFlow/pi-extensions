...
export interface PersistedOrchestratorSessionState extends OrchestratorSessionState {
	version: 1;
}
...
function parseState(entry: SessionEntry): OrchestratorSessionState | undefined {
	if (entry.type !== "custom" || entry.customType !== ORCHESTRATOR_SESSION_CUSTOM_TYPE)
		return undefined;
	if (!isRecord(entry.data)) return undefined;
	if (entry.data.version !== ORCHESTRATOR_SESSION_STATE_VERSION) return undefined;
	if (typeof entry.data.enabled !== "boolean") return undefined;
	const activeTools = Array.isArray(entry.data.activeTools)
		? entry.data.activeTools.filter((name): name is string => typeof name === "string")
		: undefined;
	return {
