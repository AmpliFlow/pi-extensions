...
export function resolveDenyEnvPatterns(
	...
	const patterns = [
		...parseDenyEnvList(agentDenyEnv),
		...parseDenyEnvList(env.PI_SUBAGENT_ENV_DENY),
	];
...
export function filterDeniedEnv<T extends Record<string, string>>(
	env: T,
	patterns: string[],
...
export function stripVolatileShellEnv<T extends Record<string, string>>(
	env: T,
): Record<string, string> {
 ...
 */
export function pickPaneIdentityEnv(
	paneEnv: Record<string, string | undefined>,
): Record<string, string> {
	const picked: Record<string, string> = {};
	for (const [key, value] of Object.entries(paneEnv)) {
		if (!value) continue;
		if (!PANE_IDENTITY_ENV_PATTERNS.some((pattern) => envNameMatchesPattern(key, pattern)))
			continue;
		picked[key] = value;
	}
	return picked;
