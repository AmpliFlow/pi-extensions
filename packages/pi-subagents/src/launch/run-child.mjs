...
try {
	...
	process.stderr.write(
		`run-child: cannot read capsule ${capsulePath}: ${error?.message ?? error}\n`,
	);
	process.exit(2);
}
...
try {
	...
	process.stderr.write(
		`run-child: cannot parse capsule ${capsulePath}: ${error?.message ?? error}\n`,
	);
	process.exit(2);
}
...
const env = mergeEnv([
	capsule.parentEnv,
	pickPaneIdentity(process.env, capsule.paneIdentityKeys),
	capsule.overrides,
]);
if (
	capsule.deriveZellijPaneSurface &&
	typeof process.env.ZELLIJ_PANE_ID === "string" &&
