...
function formatSkillsForTool(skills: Skill[], fileReadTool: SkillFileReadTool): string {
	const rendered = formatSkillsForPrompt(skills);
	return fileReadTool === "bash"
		? rendered.replace(
...
function renderOriginalSection(skills: Skill[], fileReadTool: SkillFileReadTool): string {
	...
	if (!hasCached) return formatSkillsForTool(skills, fileReadTool);
	return formatSkillsForTool(
		skills.map((skill) => {
...
function applyVisibilityAnnotations(
	skills: Skill[],
	annotations: Map<string, SkillVisibility>,
): void {
	for (const skill of skills) {
...
function replaceExactSkillSection(
	systemPrompt: string,
	current: string,
	next: string,
): string | undefined {
...
export function applySkillVisibilityToSystemPrompt(
	...
	const next = formatSkillsForTool(skills, renderTool);
