...
function gitText(args: string[], cwd: string): string {
	...
	if (r.status !== 0) {
		...
		throw new WorktreeError(
			`git ${args.join(" ")} failed (exit ${r.status}) in ${cwd}${detail ? `: ${detail}` : ""}`,
		);
	}
	return r.stdout;
}
...
export function createCandidateWorktrees(options: {
	...
	for (let index = 1; index <= options.count; index++) {
		...
		try {
			gitText(
				["worktree", "add", "-b", worktree.branch, worktree.path, preflight.baseCommit],
				preflight.repoRoot,
			);
		} catch (error) {
 ...
 */
export function snapshotCandidateWorktree(
	worktree: CandidateWorktree,
	message?: string,
): CandidateSnapshot {
	const baseTreeHash = gitText(
		["rev-parse", `${worktree.baseCommit}^{tree}`],
		worktree.path,
	).trim();
	...
	if (stagedTree === headTree) {
		...
		return {
			...
			treeHash: stagedTree,
			baseTreeHash,
			changed: stagedTree !== baseTreeHash,
		};
	}
	const configArgs = [
