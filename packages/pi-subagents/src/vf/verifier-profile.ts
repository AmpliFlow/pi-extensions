...
import type { LibraryReasoningEffort } from "./model-ref.js";
import { normalizeVerifierModelRef } from "./model-ref.js";
...
export { LIBRARY_REASONING_EFFORTS, normalizeVerifierModelRef } from "./model-ref.js";
...
export function resolveVerifierProfile(name: string, baseCwd: string): VerifierProfile {
	...
	if (existsSync(projectPath)) {
		return parseVerifierProfileSource(
			name,
...
function piEndpointFor(provider: string, modelId: string): PiModelEndpoint | null {
	...
	const providers = (modelsJson?.providers ?? {}) as Record<
		string,
		{
			baseUrl?: unknown;
			apiKey?: unknown;
			models?: Array<{ id?: unknown; baseUrl?: unknown; apiKey?: unknown }>;
		}
	>;
	...
	if (def) {
		...
		if (exact) {
			return {
				baseUrl: asString(exact.baseUrl)!,
				apiKey: literalKey(exact.apiKey) ?? literalKey(def.apiKey),
			};
		}
	...
	const entry = (
		(store?.[provider] as { models?: Array<{ id?: unknown; baseUrl?: unknown }> } | undefined)
			?.models ?? []
	).find((m) => asString(m.id) === modelId && asString(m.baseUrl));
...
function selectDoor(options: {
	...
	const specific =
		provider === "deepseek" ? "DEEPSEEK_API_KEY" : provider === "gemini" ? "VERTEX_API_KEY" : null;
