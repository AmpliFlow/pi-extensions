 ...
 * nothing.
 */
import { getEntries, type SessionEntry } from "../session/session.js";
...
function toolCallCommandBlocks(
	...
	if (name === "write") {
		const path = typeof args.path === "string" ? args.path : "";
		return [
			{ kind: "command", text: `[Command] write ${path}\n${renderWriteContent(args)}`.trimEnd() },
		];
	}
...
function applyTotalBudget(
	text: string,
	budget: number,
): { text: string; cap: TraceBudgetCap | null } {
...
export function flattenEntriesToTrace(
	entries: SessionEntry[],
	options: FlattenTraceOptions = {},
): FlattenedTrace {
	...
	const joined =
		blocks.length > 0 ? blocks.map((block) => block.text).join("\n") : EMPTY_TRACE_SENTINEL;
...
export function flattenSessionTrace(
	sessionFile: string,
	options: FlattenTraceOptions = {},
): FlattenedTrace {
