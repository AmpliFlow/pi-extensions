...
import { findLastSubagentOutputWithSource, getEntries } from "../../session/session.js";
import { flattenSessionTrace } from "../flatten.js";
import type {
	VerifiedCandidateRuntime,
	VerifiedRunManifest,
	VerifiedRunSelection,
} from "../run/types.js";
import {
	defaultVerifierCachePath,
	type MockVerifierConfig,
	runVerifierSelect,
	type VerifierSelectResponse,
	...
	return manifest.request.candidates.map((spec) => {
		...
		if (!runtime) {
			throw new SettleAbort(
				"candidate-record-missing",
				`run ${manifest.runId}: no runtime record for candidate w${spec.index}`,
			);
		}
		if (!runtime.settled) {
			throw new SettleAbort(
				"candidate-unsettled",
				`run ${manifest.runId}: candidate w${spec.index} is not settled`,
			);
		}
		...
		const trace =
			completed && !snapshotError ? safeTrace(manifest, spec.index, spec.sessionFile) : null;
		...
		return {
			spec,
			runtime,
			completed: completed && !snapshotError,
			report,
			trace,
			snapshot,
			snapshotError,
...
export async function selectWinner(
	...
	const response = await runVerifierSelect(
		{
			...
			mockVerifier:
				(manifest.request.verifier.mockVerifier as MockVerifierConfig | null | undefined) ?? null,
		},
		{
	...
	if (!winner) {
		throw new SettleAbort(
			"winner-out-of-range",
			`bridge returned winnerIndex ${response.winnerIndex} outside the distinct set`,
		);
	}
