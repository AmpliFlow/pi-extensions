...
export function atomicWriteFileSync(path: string, data: string): void {
	const tmp = join(
		dirname(path),
		`.${basenameOf(path)}.${process.pid}.${randomUUID().slice(0, 8)}.tmp`,
	);
...
export function readManifest(runDir: string): SupervisorManifest {
	...
	try {
		...
		throw new ManifestError(
			`Manifest at ${paths.manifest} is not valid JSON: ${(error as Error).message}`,
		);
	}
...
export function updateManifest(
	...
	if (promise instanceof Promise)
		throw new ManifestError("updateManifest mutator must be synchronous");
...
export function claimDelivery(
	runDir: string,
	result: SupervisorRunResult | null,
): { claimed: boolean; claim: DeliveryClaim } {
	const paths = runFilePaths(runDir);
	if (existsSync(paths.deliveryClaim)) {
		return {
			claimed: false,
			claim: JSON.parse(readFileSync(paths.deliveryClaim, "utf8")) as DeliveryClaim,
		};
	}
