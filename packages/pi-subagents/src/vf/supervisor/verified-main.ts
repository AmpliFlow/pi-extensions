...
import { ApplyError, applyWinnerToSource, cleanupRunWorktrees } from "../apply.js";
import {
...
import {
	type MockVerifierConfig,
	runVerifierProbe,
	VerifierBridgeError,
} from "../verifier/bridge.js";
import { createCandidateWorktrees, preflightWorktreeSource } from "../worktrees.js";
import { isPidAlive } from "./manifest.js";
import {
	collapseDuplicates,
	SettleAbort,
	selectWinner,
...
function provisionWorktrees(): void {
	...
	if (preflight.baseCommit !== request.baseCommit) {
		...
		log(
			runDir,
			`base commit drifted: ${request.baseCommit} -> ${preflight.baseCommit}; failing closed`,
		);
		process.exit(0);
	}
	...
	for (const worktree of worktrees) {
		...
		if (spec && spec.worktree !== worktree.path) {
			fail(
				`worktree path mismatch for candidate w${worktree.index}: planned ${spec.worktree}, created ${worktree.path}`,
			);
		}
	}
 ...
 */
async function gateVerifierCapability(): Promise<{
	coverage: string[];
	canary: { goodScore: number; badScore: number };
} | null> {
...
function allSettled(): boolean {
	return (
		manifest.candidates.length > 0 && manifest.candidates.every((candidate) => candidate.settled)
	);
}
...
function startWatchdog(): () => void {
	const interval = setInterval(() => {
		void (async () => {
			...
			if (
				current &&
				(current.leaseId !== manifest.leaseId ||
					(current.lease && current.lease.pid !== process.pid))
			) {
...
function verifierFailureCode(error: VerifierBridgeError): string {
	if (
		["capability", "credentials", "degenerate-scores", "comparison-count", "cache"].includes(
			error.kind,
		)
	) {
		return error.kind;
...
try {
	...
	while (!allSettled()) {
		...
		if (cancelled || existsSync(verifiedRunFilePaths(runDir).cancelSentinel)) {
			...
			manifest.result = failureResult(
				runStartedAt,
				"cancelled",
				`run ${manifest.runId} cancelled before selection`,
			);
	...
	try {
		({ selection, response } = await selectWinner(
			manifest,
			runDir,
			distinct,
			collapsed,
	...
	if (cancelled) {
		manifest.result = failureResult(
			runStartedAt,
			"cancelled",
			`run ${manifest.runId} cancelled during verification`,
		);
		manifest.state = "cancelled";
		writeVerifiedRunManifest(runDir, manifest);
		process.exit(0);
...
function applySelectionWinner(
	...
	try {
		...
		log(
			runDir,
			`applied winner w${selection.winnerIndex} onto base ${request.baseCommit} (tree ${applied.treeHash})`,
		);
		return {
			applied: true,
			code: "applied",
