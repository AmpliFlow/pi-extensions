...
import { getSupervisorMainPath, resolveSupervisorRuntime } from "./spawn.js";
 ...
 */
export function respawnSupervisor(
	runDir: string,
	...
	const child = spawn(
		runtime.command,
		[...runtime.preArgs, getSupervisorMainPath(), runDir, manifest.leaseId],
		{
			detached: true,
			stdio: ["ignore", logFd, logFd],
			env: options.env ?? process.env,
...
export async function cancelRun(
	...
	if (!manifest.lease || !isPidAlive(manifest.lease.pid))
		return { outcome: "not-running", manifest };
