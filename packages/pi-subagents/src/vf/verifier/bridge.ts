...
import { ensureVerifierRuntime, RUNNER_SCRIPT_PATH, VerifierRuntimeError } from "./venv.js";
...
async function runRunnerProcess(request: object, options: RunBridgeOptions): Promise<RawOutcome> {
	...
	if (options.python) {
		...
		try {
			...
			if (error instanceof VerifierRuntimeError) {
				throw new VerifierBridgeError("runtime", error.message, {
					detail: { repairSteps: error.repairSteps },
				});
			}
			throw error;
		}
	...
	try {
		...
		throw new VerifierBridgeError(
			"spawn",
			`failed to start the verifier bridge (${python}): ${(error as Error).message}`,
		);
	}
	...
	if (killedReason === "timeout") {
		throw new VerifierBridgeError(
			"timeout",
			`verifier bridge exceeded its ${timeoutMs}ms deadline and was killed`,
			{
	...
	if (killedReason === "cancelled") {
		throw new VerifierBridgeError(
			"cancelled",
			"verifier bridge was cancelled; the process group was killed",
			{
				stderrTail: stderr.slice(-STDERR_TAIL),
			},
		);
	...
	if (processError) {
		throw new VerifierBridgeError(
			"spawn",
			`verifier bridge process failed to start (${python}): ${processError}`,
		);
	}
...
function throwMappedError(
	outcome: RawOutcome,
	fallbackKind: VerifierBridgeErrorKind,
	context: string,
): never {
	...
	if (
		...
		response.ok === false &&
		response.error &&
		typeof response.error.kind === "string"
	) {
	...
	throw new VerifierBridgeError(
		fallbackKind,
		`${context}: bridge exited with code ${outcome.exitCode}${signal}${suffix}`,
		{
			exitCode: outcome.exitCode,
			detail,
			stderrTail: outcome.stderrTail,
...
function assertSelectRequest(request: VerifierSelectRequest): void {
	...
	if (!Array.isArray(request.candidates) || request.candidates.length === 0) {
		...
		request.candidates.forEach((trace, index) => {
			if (typeof trace !== "string" || !trace.trim())
				problems.push(`candidates[${index}] must be a non-empty string`);
		});
	}
	...
	if (problems.length > 0) {
		throw new VerifierBridgeError(
			"malformed-request",
			`invalid select request: ${problems.join("; ")}`,
		);
	}
}
...
function validateSelectResponse(response: unknown, candidateCount: number): VerifierSelectResponse {
	...
	const problems: string[] = [];
	if (!r || r.ok !== true || r.kind !== "select")
		problems.push("response is not an ok select result");
	if (
	...
	if (
		...
		r.ranking.length !== candidateCount ||
		new Set(r.ranking).size !== candidateCount
	) {
		problems.push("ranking is not a permutation of the candidate indices");
	}
	if (
		!Array.isArray(r?.scores) ||
		r.scores.length !== candidateCount ||
		r.scores.some((s) => typeof s !== "number")
	) {
	...
	if (
		...
		r.criteria.length === 0 ||
		r.criteria.some((c) => typeof c !== "string" || !c)
	) {
	...
	if (problems.length > 0) {
		throw new VerifierBridgeError(
			"protocol",
			`select response failed validation: ${problems.join("; ")}`,
		);
	}
...
export async function runVerifierSelect(
	...
	if (
		outcome.response &&
		(outcome.response as { ok?: boolean }).ok === true &&
		outcome.exitCode === 0
	) {
...
function assertProbeRequest(request: VerifierProbeRequest): void {
	...
	if (problems.length > 0) {
		throw new VerifierBridgeError(
			"malformed-request",
			`invalid probe request: ${problems.join("; ")}`,
		);
	}
}
...
function validateProbeResponse(response: unknown): VerifierProbeResponse {
	...
	const problems: string[] = [];
	if (!r || r.ok !== true || r.kind !== "probe")
		problems.push("response is not an ok probe result");
	if (typeof r?.model !== "string" || !r.model) problems.push("model echo missing");
	...
	if (problems.length > 0) {
		throw new VerifierBridgeError(
			"protocol",
			`probe response failed validation: ${problems.join("; ")}`,
		);
	}
