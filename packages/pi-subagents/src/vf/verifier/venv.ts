...
import { getAgentConfigDir } from "../../agents/definitions.js";
...
function validateInstalled(python: string): {
	ok: boolean;
	version: string | null;
	error: string | null;
} {
	const probe = runCaptured(
		python,
		["-c", "import llm_verifier; print(llm_verifier.__version__)"],
		{
			timeoutMs: 30_000,
		},
	);
	...
	if (version !== LLM_VERIFIER_VERSION) {
		return {
			ok: false,
			version,
			error: `installed llm-verifier ${version} != pinned ${LLM_VERIFIER_VERSION}`,
		};
	}
...
function readMarker(root: string): Marker | null {
	try {
		...
		if (parsed && typeof parsed.version === "string" && typeof parsed.provisioner === "string")
			return parsed;
	} catch {
...
function installWithUv(root: string, uv: string): { provisioner: string; python: string } {
	...
	if (create.status !== 0) {
		throw new VerifierRuntimeError(
			`uv venv failed to create the verifier venv:\n${tail(create.stderr)}`,
			[
...
function installWithPythonVenv(
	root: string,
	python3: string,
): { provisioner: string; python: string } {
	const venvDir = join(root, "venv");
	const create = runCaptured(python3, ["-m", "venv", venvDir]);
	if (create.status !== 0) {
		throw new VerifierRuntimeError(
			`python3 -m venv failed to create the verifier venv:\n${tail(create.stderr)}`,
			[
...
export function ensureVerifierRuntime(options: { timeoutMs?: number } = {}): VerifierRuntimeInfo {
	if (process.platform === "win32") {
		throw new VerifierRuntimeError("Verified fan-out supports Linux and macOS only.", [
			"Run from a Linux or macOS host.",
		]);
	}
	...
	if (marker && marker.version === LLM_VERIFIER_VERSION && existsSync(python)) {
		...
		if (validation.ok) {
			return {
				...
				python,
				provisioner: marker.provisioner,
...
function waitForMarker(root: string, timeoutMs: number): boolean {
	...
	while (Date.now() < deadline) {
		...
		if (
			marker &&
			marker.version === LLM_VERIFIER_VERSION &&
			existsSync(venvPython(join(root, "venv")))
		)
			return true;
