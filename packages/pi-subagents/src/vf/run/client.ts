...
import { isPidAlive } from "../supervisor/manifest.js";
import { resolveSupervisorRuntime } from "../supervisor/spawn.js";
import { defaultVerifierCachePath } from "../verifier/bridge.js";
import {
	isTerminalRunState,
...
export function respawnVerifiedSupervisor(
	runDir: string,
	...
	if (isTerminalRunState(manifest.state)) {
		throw new Error(
			`Cannot respawn supervisor: run ${manifest.runId} is already ${manifest.state}.`,
		);
	}
...
export function retryVerifiedRunVerification(
	...
	if (manifest.result?.ok) {
		throw new Error(
			`Cannot retry verification: run ${manifest.runId} already completed with a winner.`,
		);
	}
	const failure = manifest.result?.failure;
	if (
		!failure ||
		!(RETRYABLE_VERIFICATION_FAILURE_CODES as readonly string[]).includes(failure.code)
	) {
		throw new Error(
	...
	if (options.verifier) {
		...
		if (!merged.model?.trim() || !merged.criteriaPath?.trim()) {
			throw new Error(
				"Cannot retry verification: the verifier override must keep a non-empty model and criteriaPath.",
			);
		}
	...
	return new Promise((resolve, reject) => {
		...
		const poll = () => {
			...
			if (Date.now() > deadline) {
				...
				reject(
					new Error(
						`Timed out waiting for verified run ${manifest.runId} (state ${manifest.state}).`,
					),
				);
				return;
			}
