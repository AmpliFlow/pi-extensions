...
import { atomicWriteFileSync, isPidAlive } from "../supervisor/manifest.js";
...
export function readVerifiedRunManifest(runDir: string): VerifiedRunManifest {
	...
	try {
		...
		const reason =
			(error as { code?: string }).code === "ENOENT" ? "does not exist" : "is not valid JSON";
		throw new VerifiedRunError(
			`Verified-run manifest at ${manifest} ${reason} (${(error as Error).message}).`,
		);
	}
	if (
		parsed.version !== VERIFIED_RUN_MANIFEST_VERSION &&
		parsed.version !== LEGACY_VERIFIED_RUN_MANIFEST_VERSION
	) {
		throw new VerifiedRunError(
	...
	if (parsed.runId !== basename(runDir)) {
		throw new VerifiedRunError(
			`Verified-run manifest at ${manifest} names run ${parsed.runId} but lives in ${runDir}.`,
		);
	}
	if (!parsed.request || parsed.request.kind !== "verified-fanout") {
		throw new VerifiedRunError(
			`Verified-run manifest at ${manifest} is missing its verified-fanout request.`,
		);
	}
	return parsed;
}
 ...
 */
export function claimVerifiedRunDelivery(runDir: string): {
	claimed: boolean;
	claimedByPid: number;
} {
	const paths = verifiedRunFilePaths(runDir);
	...
	if (existsSync(paths.deliveryLease)) {
		try {
			const lease = JSON.parse(
				readFileSync(paths.deliveryLease, "utf8"),
			) as VerifiedRunDeliveryLease;
...
export function writeVerifiedRunDeliveryReceipt(
	...
	const receipt: VerifiedRunDeliveryReceipt = {
		...info,
		digest: createHash("sha256")
			.update(JSON.stringify(manifest.result ?? null))
			.digest("hex"),
		deliveredAt: new Date().toISOString(),
	};
	writeFileSync(
		verifiedRunFilePaths(runDir).deliveryReceipt,
...
export function readVerifiedRunDeliveryReceipt(runDir: string): VerifiedRunDeliveryReceipt | null {
	try {
		return JSON.parse(
			readFileSync(verifiedRunFilePaths(runDir).deliveryReceipt, "utf8"),
		) as VerifiedRunDeliveryReceipt;
	} catch {
		return null;
	}
}
