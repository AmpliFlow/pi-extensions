...
import { resolveArtifactProjectRoot } from "../../artifact-storage.js";
import { outstandingWork } from "../../runtime/outstanding-work.js";
import {
	claimSpawnWidthSlot,
	getSpawnWidthLimit,
	releaseSlots,
...
import { moduleAbortController, runningSubagents } from "../../runtime/state.js";
import type { RunningSubagent, SubagentResult } from "../../types.js";
import {
	attachVerifiedRun,
	isVerifiedRunOrphaned,
	listVerifiedRuns,
	respawnVerifiedSupervisor,
	waitForVerifiedRunResult,
} from "./client.js";
import { verifiedRunsBaseDir, verifiedRunToSubagentResult } from "./launch.js";
import {
...
export async function adoptVerifiedRuns(
	...
	for (const runDir of listVerifiedRuns(verifiedRunsBaseDir(cwd))) {
		let snapshot: ReturnType<typeof attachVerifiedRun>;
		try {
		...
		if (
			resolveArtifactProjectRoot(manifest.request.sourceRepo) !== resolveArtifactProjectRoot(cwd)
		) {
...
function registerAdoptedWatcher(
	...
	running.completionPromise = releaseSpawnWidthSlotOnCompletion(
		...
		(async (): Promise<SubagentResult> => {
			try {
				let final: Awaited<ReturnType<typeof waitForVerifiedRunResult>>;
				try {
					...
					return {
						...
						task: manifest.request.taskPrompt,
						summary:
							`[llm-as-a-verifier ${manifest.runId}: this session stopped watching; ` +
...
async function deliverWithReceipt(
	...
	if (persisted) {
		try {
			writeVerifiedRunDeliveryReceipt(runDir, {
...
function deliverVerifiedRunResult(
	...
	outstandingWork.delivery(manifest.runId.slice(-8), () =>
		pi.sendMessage(
			{
