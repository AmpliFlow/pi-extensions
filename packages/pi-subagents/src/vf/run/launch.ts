...
import { getArtifactProjectName, getArtifactStorageRoot } from "../../artifact-storage.js";
import { buildBackgroundLaunchPlan } from "../../launch/background.js";
import { buildParentEnvSnapshot } from "../../launch/child-env.js";
...
import { runningSubagents } from "../../runtime/state.js";
...
import { resolveVerifiedFanOutLaunch, VerifierCriteriaError } from "../criteria.js";
import { newRunId } from "../supervisor/run-client.js";
import { resolveVerifierModel, VerifierProfileError } from "../verifier-profile.js";
import {
...
import { startVerifiedRun, verifiedRunDirFor, waitForVerifiedRunResult } from "./client.js";
import type { VerifiedCandidateSpec, VerifiedRunManifest, VerifiedRunRequest } from "./types.js";
import {
...
function readSessionHeader(
	file: string,
): { type?: unknown; id?: unknown; parentSession?: unknown } | null {
	try {
...
export function resolveAuthorizedRecipients(
	...
	for (let depth = 0; current && depth < MAX_LINEAGE_DEPTH; depth += 1) {
		...
		if (!parentHeader || parentHeader.type !== "session" || typeof parentHeader.id !== "string")
			break;
...
export async function launchVerifiedFanOut(
	...
	try {
		...
		verifier = resolveVerifierModel({
			override: fanout.modelOverride ?? undefined,
			baseCwd: ctx.cwd,
		});
	...
	try {
		...
		for (let index = 1; index <= fanout.candidates; index++) {
			...
			if (index === 1) {
				...
				if (
					invocationArgs.slice(invocationArgs.length - plan.args.length).join("\u0000") !==
					plan.args.join("\u0000")
				) {
					throw new VerifiedLaunchError(
...
async function waitForVerifiedOutcome(
	...
	try {
		...
		if (options.signal.aborted) {
			...
			const settled = await waitForVerifiedRunResult(runDir, { timeoutMs: 10_000 }).catch(
				() => null,
			);
