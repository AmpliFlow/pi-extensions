...
import { normalizeVerifierModelRef } from "../vf/model-ref.js";
...
function parseAgentDefinition(
	...
	const llmAsVerifier = parseLlmAsVerifier(
		get("llm-as-a-verifier"),
		hasKey("llm-as-a-verifier"),
		path,
	);
...
function parseLlmAsVerifier(
	...
	throw new Error(
		`llm-as-a-verifier must be "true" or "false" (got ${JSON.stringify(raw ?? "")}) in ${path}.`,
	);
}
...
function parseLlmAsVerifierModel(
	...
	if (raw === undefined || !raw.trim()) {
		throw new Error(
			`llm-as-a-verifier-model must be a verifier profile name or provider/model[:thinking] (got ${JSON.stringify(raw ?? "")}) in ${path}.`,
		);
	}
	try {
...
export function getEffectiveAgentDefinitions(baseCwd = process.cwd()): ResolvedAgentDefinition[] {
	...
	const packageDir = join(dirname(fileURLToPath(import.meta.url)), "..", "..", "agents");
	const dirs = [
		{
			path: packageDir,
			source: "package" as const,
			cwdBase: dirname(packageDir),
		},
		{
