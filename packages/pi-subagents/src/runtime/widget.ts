...
import { getSubagentActivityStartIndex } from "../session/session-files.js";
import type {
	RunningSubagent,
	SessionContentBlock,
	SessionEntryLike,
...
import { readVerifiedRunDisplay, type VerifiedCandidateDisplay } from "../vf/widget.js";
...
export class SubagentWidgetManager {
	...
	private candidateStats(
		...
		candidate: VerifiedCandidateDisplay,
	): RunningSubagent {
		...
		if (agents.length > 0) {
			...
			for (let i = 0; i < visibleAgents.length; i++) {
				...
				if (display) {
					...
					verifiedGroupsLeft--;
					const group = this.renderVerifiedGroup(
						theme,
						spinner,
						agent,
						display,
	...
	private renderVerifiedGroup(
		...
		if (display.state === "verifying") {
			rows.push({
				label: "verifier",
				detail: [`ranking ${display.candidateCount} traces…`],
				dim: false,
			});
		}
