...
import { loadAgentDefaults as loadAgentDefaultsFromDefinitions } from "../agents/definitions.js";
import { assertModelAllowed, buildModelRef, splitModelRef } from "../agents/model-refs.js";
import { getArtifactStorageRoot } from "../artifact-storage.js";
import { buildAppendSystemInheritancePlan } from "../launch/append-system.js";
import { getPiInvocation, getSubagentChildProcessEnv } from "../launch/child-command.js";
import { resolveDenyEnvPatterns } from "../launch/child-env.js";
import { CHILD_CONTEXT_BOUNDARY_SYSTEM_PROMPT } from "../launch/context-boundary.js";
import { parseEnvString } from "../launch/env.js";
import { resolveSubagentTimeoutState } from "../launch/policy.js";
import {
...
import { writeResumeTaskArtifact } from "../launch/prompt-artifacts.js";
import { buildResumePiArgs, getResumeCwd, resolveResumeLaunchMetadata } from "../launch/resume.js";
import { resolveSubagentCwd } from "../launch/runtime-paths.js";
import { buildInteractiveShellCommand } from "../launch/shell-command.js";
import {
	getSkillVisibilitySpec,
	PI_SUBAGENT_SKILL_VISIBILITY,
} from "../launch/skill-visibility.js";
import { expandSubagentTask } from "../launch/task-expansion.js";
import { createZellijCommandSurface } from "../mux/zellij-placement.js";
import { getZellijShellCommand, resolveZellijTarget } from "../mux/zellij-runtime.js";
import {
	createSurface,
	getMuxBackend,
	muxSetupHint,
...
import { clearSubagentExitSidecar } from "../session/exit-sidecar.js";
import { getEntryCount } from "../session/session.js";
import {
	getDoneSentinelFile,
	isResumeMode,
...
} from "../session/timeout-sidecar.js";
import {
	buildResumeSpawnEnv,
	narrowSpawnBudget,
	parseSpawnEnv,
...
} from "../tools/context-reminders.js";
import {
	PI_SUBAGENT_IDLE_TIMEOUT,
	PI_SUBAGENT_TIMEOUT,
	PI_SUBAGENT_TIMEOUT_STARTED_AT,
	PI_SUBAGENT_TIMEOUT_WARN_THRESHOLD,
} from "../tools/timeout-reminders.js";
import type { RunningSubagent, SubagentResult } from "../types.js";
import {
	claimSpawnWidthSlot,
	getLiveSlotCount,
	getSpawnWidthLimit,
...
async function resumeSubagentSessionWithoutWidth(
	...
	if (metadata.mode === "background") {
		...
		const child = spawn(invocation.command, invocation.args, {
			...
			env: getSubagentChildProcessEnv(
				invocation,
				resumeEnvVars,
				resolveDenyEnvPatterns(invocationMetadata?.denyEnv),
			),
		});
	...
	if (shouldPersistInvocationMetadata) {
		...
		if (resumedInvocationMetadata)
			writeSubagentLaunchMetadataEntry(sessionFile, resumedInvocationMetadata);
	}
