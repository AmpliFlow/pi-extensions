import { ORCHESTRATOR_ALLOWED_TOOL_NAMES } from "../tools/tool-names.js";
...
export function normalizeOrchestratorTools(toolNames: readonly string[]): string[] {
	return [...new Set(toolNames.filter((name) => typeof name === "string" && name.length > 0))];
}

/** Compare normalized tool snapshots in their registered order. */
export function sameOrchestratorTools(left: readonly string[], right: readonly string[]): boolean {
...
export function currentToolsAreControllerRestricted(
	...
	return previousMode && currentTools.every((name) => ORCHESTRATOR_ALLOWED_TOOL_NAMES.has(name));
}
...
export function deriveNormalToolsOnDisable(
	...
	if (liveTools.some((name) => !ORCHESTRATOR_ALLOWED_TOOL_NAMES.has(name))) return liveTools;

	const expected =
		modeActiveTools.length > 0 ? modeActiveTools : filterOrchestratorTools(normalActiveTools);
	const missingAllowed = new Set(expected.filter((name) => !liveTools.includes(name)));
