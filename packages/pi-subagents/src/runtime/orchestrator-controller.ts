...
} from "../session/orchestrator-state.js";
import {
...
import { ORCHESTRATOR_BASE_PROMPT } from "./orchestrator-prompt.js";
import { runningSubagents } from "./state.js";
...
type OrchestratorEffectiveDefaultSource = OrchestratorEffectiveDefault["source"];
...
export interface OrchestratorController {
	...
	setMode(enabled: boolean, ctx?: OrchestratorContext): OrchestratorOperationResult;
	saveGlobalDefault(enabled: boolean, ctx?: OrchestratorContext): OrchestratorOperationResult;
	...
	handleToolCall(event: Pick<ToolCallEvent, "toolName">): ToolCallEventResult | undefined;
}
...
export function createOrchestratorController(
	...
	const getRunningSubagentCount = options.getRunningSubagentCount ?? (() => runningSubagents.size);
	...
	function refreshGlobalDefaults(): void {
		...
		effectiveDefault = resolveOrchestratorEffectiveDefault(environment, agentDir);
	}
	...
	function notifyConfigurationIssue(ctx: OrchestratorContext | undefined): void {
	...
	function getGuardState(ctx: OrchestratorContext | undefined): {
		...
		if (!parentIdle) return { parentIdle, hasPendingMessages, blockedReason: "parent-busy" };
	...
	function getSnapshot(ctx?: OrchestratorContext): OrchestratorSnapshot {
		...
		return {
			...
			effectiveGlobalDefaultSource: childSession ? "missing" : effectiveDefault.source,
	...
	function applyBranchState(ctx: OrchestratorContext, persistMissing: boolean): void {
		...
		if (shouldPersist && !persistSessionState(ctx, enabled, baseline)) {
			...
			normalActiveTools = currentMode ? [...normalActiveTools] : [...currentTools];
			return;
		}
	...
	function restorePreviousState(
		...
		modeActiveTools = previousMode ? [...filterOrchestratorTools(previousActiveTools)] : [];
		try {
	...
	const controller: OrchestratorController = {
		...
		setMode(enabled, nextContext) {
			...
			if (enabled === currentMode) return { ok: true, changed: false, snapshot: before };
			...
			const nextNormalTools = enabled
				? [...previousActiveTools]
				: deriveNormalToolsOnDisable(previousActiveTools, normalActiveTools, modeActiveTools);
			...
			try {
				...
				restorePreviousState(previousMode, previousNormalTools, previousActiveTools);
				persistSessionState(operationContext, previousMode, previousNormalTools, false);
		...
		saveGlobalDefault(enabled, nextContext) {
			...
			if (!result.ok) {
				...
				notify(nextContext ?? context, CONFIGURATION_ISSUE_PREFIX + error, "error");
				return {
					ok: false,
					changed: false,
					snapshot: getSnapshot(nextContext),
		...
		handleSessionShutdown(nextContext) {
			...
			if (persistSessionState(nextContext, currentMode, normalActiveTools, false))
				baselineDirty = false;
		},
		beforeAgentStart(event) {
			...
			const currentTools = getCurrentTools();
			if (currentTools.some((name) => !filterOrchestratorTools([name]).length)) {
				normalActiveTools = [...currentTools];
				baselineDirty = true;
			}
			const allowedTools = filterOrchestratorTools(currentTools);
			if (!sameOrchestratorTools(currentTools, allowedTools)) setCurrentTools(allowedTools);
		...
		handleToolCall(event) {
			if (childSession || !currentMode || filterOrchestratorTools([event.toolName]).length > 0)
				return undefined;
			return {
				block: true,
				reason: "Orchestrator mode only permits delegation and orchestrator-control tools.",
			};
		},
	};
