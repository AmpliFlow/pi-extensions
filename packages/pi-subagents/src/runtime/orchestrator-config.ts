...
type OrchestratorEffectiveDefaultSource = "env" | "saved" | "missing" | "invalid-env" | "error";
...
export function loadOrchestratorGlobalConfig(agentDir = getAgentDir()): OrchestratorGlobalConfig {
...
export function resolveOrchestratorEffectiveDefault(
	...
	if (config.source === "saved") return { value: config.savedDefault, source: "saved" };
	if (config.source === "error") return { value: false, source: "error", error: config.error };
