...
import { getPackagedCriteriaDir } from "../../src/vf/criteria.js";
import { startVerifiedRun } from "../../src/vf/run/client.js";
import {
	isTerminalRunState,
	readVerifiedRunManifest,
	type VerifiedRunManifest,
} from "../../src/vf/run/types.js";
import { newRunId } from "../../src/vf/supervisor/run-client.js";
import {
	candidateWorktreeBranchName,
	candidateWorktreeDirName,
	preflightWorktreeSource,
} from "../../src/vf/worktrees.js";
import { createTestDir, sleep, writeExecutable } from "./fixtures.js";
...
export function setupVerifiedRunFixture(): {
	repo: string;
	parent: string;
	fakePi: string;
	captureDir: string;
...
export async function waitForVerifiedRunState(
	runDir: string,
	state: string,
	...
	for (;;) {
		...
		if (isTerminalRunState(manifest.state)) {
			throw new Error(
				`run reached ${manifest.state} while waiting for ${state}: ${manifest.result?.failure?.message}`,
			);
		}
