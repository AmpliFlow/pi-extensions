...
describe("subagent launch result delivery", () => {
	...
	it("records a headless forced-sync launch as a blocking batch so the completed report is read", async () => {
		...
		const runtime: SubagentToolRuntime = {
			...
			startWidgetRefresh: () => {},
			getLaunchedSubagentResult: (running, signal) =>
				getLaunchedSubagentResultForTest(running, signal),
	...
	it("injects one hidden startup catalog for top-level actionable sessions", () => {
		...
		assert.ok((message.details as any).entries.some((entry: any) => entry.name === "reviewer"));
		assert.equal(
		...
		assert.match(
			message.content,
			/`reviewer`: Review changes for regressions[\s\S]*?tool_return: wait_here/m,
		);
