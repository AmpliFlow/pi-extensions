...
} from "../../src/runtime/result-router.js";
import { buildCompletedSubagentResult, requestSubagentBatchStop } from "../../src/runtime/state.js";
...
describe("result router", () => {
	...
	it("routes detached completion through one parent-visible result", () => {
		...
		assert.equal(
			sent[0].message.display,
			true,
			"parent-visible completion messages must be displayed",
		);
	...
	it("does not redeliver an awaited or already delivered completion", () => {
		...
		const awaited = buildCompletedSubagentResult(
			makeRunning({ deliveryState: "awaited" }),
			makeResult(),
		);
		const deliveredAwaited = deliverCompletedSubagentResult(
			pi,
			awaited,
			(seconds) => `${seconds}s`,
		);
		...
		const deliveredAgain = deliverCompletedSubagentResult(
			pi,
			alreadyDelivered,
			(seconds) => `${seconds}s`,
		);
	...
	it("keeps optional delivery facts in structured completion details", () => {
		...
		assert.deepEqual(sent[0].message.details.timeoutWrapUp, {
			kind: "timeout",
			seconds: 20,
			threshold: 80,
		});
