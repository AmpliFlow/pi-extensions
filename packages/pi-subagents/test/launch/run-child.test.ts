...
import { PANE_IDENTITY_ENV_PATTERNS } from "../../src/launch/child-env.js";
import { writeEnvCapsule } from "../../src/launch/env-capsule.js";
import { assert, createTestDir, describe, it, readFileSync, rmSync } from "../support/index.js";
...
function runLauncher(
	capsule: Parameters<typeof writeEnvCapsule>[0],
...
describe("run-child launcher", () => {
	it("merges parent snapshot, pane identity, and overrides in precedence order", () => {
		...
		assert.equal(
			childEnv.DAEMON_VAR,
			undefined,
			"non-identity pane env must not leak into the child",
		);
	...
	it("unlinks a malformed capsule before exiting with an error", () => {
		const dir = join(
			tmpdir(),
			`pi-subagent-env-badcaps-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
