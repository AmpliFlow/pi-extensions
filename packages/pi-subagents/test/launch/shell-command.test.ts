...
import {
	...
	getRunChildLauncherPath,
} from "../../src/launch/shell-command.js";
import {
	assert,
	createTestDir,
	describe,
...
describe("interactive shell command builder", () => {
	...
	it("filters the parent snapshot with deny-env patterns and strips shell bookkeeping", () => {
		...
		try {
			...
			const capsule = JSON.parse(readFileSync(capsulePath, "utf8"));
			assert.equal(
				capsule.parentEnv.TEST_DENY_A,
				undefined,
				"global deny must filter the snapshot",
			);
			assert.equal(
				capsule.parentEnv.TEST_KEEP_B,
				undefined,
				"agent deny-env must filter the snapshot",
			);
